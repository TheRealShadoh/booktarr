/**
 * Clean Barcode Scanner Implementation
 * Uses ZXing for barcode detection without video control conflicts
 */
import React, { useRef, useEffect, useState, useCallback } from 'react';
import { BrowserMultiFormatReader, DecodeHintType, BarcodeFormat } from '@zxing/library';
import { useAppContext } from '../context/AppContext';

interface BarcodeScannerNewProps {
  onComplete: (isbns: string[]) => void;
  onClose: () => void;
}

const BarcodeScannerNew: React.FC<BarcodeScannerNewProps> = ({ onComplete, onClose }) => {
  ...
  const extractISBN = useCallback((text: string): string | null => {
    ...
    // Accept any 13-digit code as potential book barcode
    if (digits.length === 13) {
      return digits;
    }
  ...
  }, [scannedISBNs, playBeep, extractISBN, showToast]);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-booktarr-textSecondary mt-1">
              {scannedISBNs.length} ISBN{scannedISBNs.length !== 1 ? 's' : ''} scanned
            </p>
          </div>
          <button
            onClick={onClose}
